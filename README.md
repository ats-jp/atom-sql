...
SQLの編集を行う場合: `jp.ats.atomsql.Atom`、`jp.ats.atomsql.Protoatom`  
...
/**
 * SQL実行の中間状態である jp.ats.atomsql.Protoatom として返却
...
- 検索結果が単一項目の値の取得  
以下表の型は、`java.util.List`、`java.util.stream.Stream`、`java.util.Optional`、`jp.ats.atomsql.Atom`、`jp.ats.atomsql.Protoatom`の型パラメータに直接型名を記述することで、検索結果の先頭項目をその型の値として取得することが可能となる  
...
戻り値の型は`int`、`void`もしくは`jp.ats.atomsql.Atom`、`jp.ats.atomsql.Protoatom`である必要がある  
